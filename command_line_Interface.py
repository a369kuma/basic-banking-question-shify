...
Available Commands:
  ...
  deposit [amt] - Deposit an amount into the account. Example: deposit 100
  withdraw [amt]- Withdraw an amount from the account. Example: withdraw 50
...
def main():
    ...
    while True:
        ...
        elif command.startswith("deposit") or command.startswith("dep"):
            try:
                amount = float(command.split()[1])
                account.deposit(amount)
            except (IndexError, ValueError):
                print("Invalid deposit command. Use: deposit [amount]")
        elif command.startswith("withdraw") or command.startswith("wd"):
            try:
                amount = float(command.split()[1])
                account.withdraw(amount)
            except (IndexError, ValueError):
                print("Invalid withdraw command. Use: withdraw [amount]")
        else:
